...
import torchvision    
def non_max_suppression(prediction, conf_thres=0.4, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    locs = prediction[:,:,:4]
    probs = prediction[:,:,4:]
    probs[:, 2:] *= probs[:, 1:2]  # conf = obj_conf * cls_conf 
    locs, probs = xywh_to_xyxy(locs, probs)
    ...
    flat_probs = probs.contiguous().view(-1)
    ...
    threshold_mask = (flat_probs > conf_thres) & (class_indexes > 0)
    ...
    if bboxes.size(0) > 0:
       print(bboxes.size(), class_indexes.size(), probs.size())
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            try:  # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
                iou = box_iou(boxes[i].cpu(), boxes) > iou_thres.cpu()  # iou matrix
                weights = iou.cpu() * scores[None]  # box weights
                x[i, :4] = torch.mm(weights, x[:, :4]).float() / weights.sum(1, keepdim=True)  # merged boxes
                if redundant:
                    i = i[iou.sum(1) > 1].cpu()  # require redundancy
